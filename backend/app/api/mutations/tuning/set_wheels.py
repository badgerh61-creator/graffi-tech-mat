...
def set_wheels(*, db, user, payload: dict):
    """
    Phase K.2 — Set wheels
    Mirrors set_suspension_preset logic EXACTLY.
    """
    ...
    ok, reason = validate_wheel_parameters(
        diameter=diameter,
        width=width,
        offset=offset,
    )
    ...
    scene_state_hash = _hash_scene_and_tuning(
        scene_state={},          # K.2 does NOT mutate scene
    ...
    entry = JournalEntry(
        project_id=project_id,
        mutation_type="tuning.set-wheels",  # EXACT string
