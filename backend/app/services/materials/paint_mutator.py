...
def _ensure_decor(snapshot) -> Dict[str, Any]:
    ...
    if "paint_swatches" not in decor or not isinstance(decor.get("paint_swatches"), list):
...
def _find_swatch(swatches: List[Dict[str, Any]], swatch_id: str) -> Optional[Dict[str, Any]]:
    for s in swatches:
...
def apply_delete_swatch(snapshot, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"ok": True, "removed": before - len(swatches), "swatch_id": swatch_id}
