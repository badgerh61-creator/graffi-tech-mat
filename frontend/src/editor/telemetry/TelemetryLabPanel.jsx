...
import { createScenario, listScenarios, runScenario, listRuns, compareMatrix } from "../../services/simulationLabApi";
    ...
    <div className="border rounded p-3 space-y-3">
      ...
      <div className="border rounded p-2 space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <input className="border rounded px-2 py-1 text-sm" type="number" value={newThrottle} onChange={(e) => setNewThrottle(e.target.value)} />
        </div>
        <button className="border rounded px-3 py-1 text-sm" disabled={busy || !projectId} onClick={onCreateScenario}>
          Save Scenario
        </button>
      </div>
      ...
      <div className="border rounded p-2 space-y-2">
        ...
        <select className="border rounded px-2 py-1 text-sm w-full" value={selectedScenarioId} onChange={(e) => setSelectedScenarioId(e.target.value)}>
        ...
        <button className="border rounded px-3 py-1 text-sm" disabled={busy || !activeSnapshot?.id || !selectedScenarioId} onClick={onRunScenario}>
      ...
      <div className="border rounded p-2 space-y-2">
        ...
        <div className="space-y-1">
          {runs.map((r) => (
            <label key={r.run_id} className="text-xs flex items-center gap-2">
              <input type="checkbox" checked={selectedArtifacts.includes(r.artifact_id)} onChange={() => toggleArtifact(r.artifact_id)} />
              run:{r.run_id} artifact:{r.artifact_id} snap:{r.snapshot_id} scenario:{r.scenario_id ?? "—"} engine:{r.engine_version}
            </label>
          ))}
        </div>

        <button className="border rounded px-3 py-1 text-sm" disabled={busy || selectedArtifacts.length < 2} onClick={onBuildMatrix}>
